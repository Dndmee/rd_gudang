...
ESX.RegisterServerCallback('rd_gudang:checkLocker', function(source, cb, lockerId)
	local braveid = source
	local xPlayer = ESX.GetPlayerFromId(braveid)
...
AddEventHandler('rd_gudang:startRentingLocker', function(lockerId, lockerName) 
	...
	local xPlayer = ESX.GetPlayerFromId(braveid)
			if xPlayer.getMoney() >= RD.InitialRentPrice then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', braveid, { type = 'success', text = "Kamu mulai menyewa " ..lockerName.. ". Kamu akan dikenakan biaya Rp."..RD.DailyRentPrice.." harian (IDR)", length = 5000 })
			else
				TriggerClientEvent('mythic_notify:client:SendAlert', braveid, { type = 'error', text = "Kamu tidak memiliki uang untuk membayar biaya penyewaan pertama.", length = 5000 })
			end
	end)
...
AddEventHandler('rd_gudang:stopRentingLocker', function(lockerId, lockerName) 
	...
	MySQL.Async.fetchAll('SELECT * FROM rd_gudang WHERE lockerName = @lockerId AND identifier = @identifier', { ['@lockerId'] = lockerId, ['@identifier'] = xPlayer.identifier }, function(result)
		if result[1] ~= nil then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', braveid, { type = 'inform', text = "Membatalkan penyewaan gudang.", length = 5000 })
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', braveid, { type = 'error', text = "Kamu tidak memiliki gudang ini.", length = 5000 })
		end
	end)
end)
